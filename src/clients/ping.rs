...
impl Ping {
    pub fn new(address: &str, count: u32, interval: u64) -> Self {
        Ping {
    ...
    pub async fn start(&self) {
        for i in 0..self.count {
            match tokio::net::TcpStream::connect(&self.address).await {
                Ok(socket) => Self::process_ping(socket, i, &self.address).await,
    ...
    async fn process_ping(mut socket: tokio::net::TcpStream, seq: u32, address: &str) {
